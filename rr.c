...
CastType(que_char* q)
{
    ...
    if(Equal(&t, "f64")) type = F64; else quit("type '%s' not supported\n", t.value);
    Match(q, '>');
    str_free(&t);
    return type;
}
...
{
    return (Next(q) == '(') ? Parn(q, idents) : Term(q, idents);
}
...
main(void)
{
    Execute(
        "let Z = { \"list\" :"
                "[<f64> 1, <i32> 1, <i32> 2, <i32> 3]"
              "+ [<i64> 2,  <u8> 1,  <u8> 2,  <u8> 3] };"
        "let thing = true;"
        "let array = ["
            "<u32> 128,"
            "<u32> 128,"
